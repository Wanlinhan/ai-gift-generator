...
本地历史记录会写入 `data/app.db`，生成的图片/视频文件会写入 `uploads/`。这两个目录已被 `.gitignore` 忽略，不会上传到 GitHub。
...
Docker 模式下，`./data` 和 `./uploads` 会挂载到容器内保存 SQLite 数据库和生成文件，容器重建后仍然保留。
